...
from (
    select if (toHour(view_dttm) >=18 and toHour(view_dttm) < 20, geoToH3(del_longitude, del_latitude, 7), null)
            h3_to,
           count(*) as incomes_to
    from data_analysis.views v
        join data_analysis.orders o on o.idhash_order = v.idhash_order
...
join (
    select if (toHour(view_dttm) >=8 and toHour(view_dttm) < 10, geoToH3(longitude, latitude, 7), null) as h3_from,
